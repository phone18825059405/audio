<template>
    <div class="container">
        <div class="playTit">
           ...
           <span class="backTex">搜索</span><span class="result"></span>
        </div>
        <div class="singList" id="searchcommbtnid">
           <div class="seackBox">
                <p class="seackLe"><input type="text" ref="searchBar" id="searchvalueId2" class="seack" placeholder="歌手/歌名/歌词" value=""></p>
                <a href="javascript:" class="seackBtn" @click="loadMusicList('true',$event)">搜索</a>
           </div>
           <p class="downApp">
           </p>
           <transition name="fade">
               <div class="seackCov" v-if="searchKeys==''">
                    <p class="seackTle" id="lefthotid"><span v-for="item in hotKeyList.left" v-tap="{ methods : loadHotKey }">{{item.name}}</span></p>
                    <p class="seackTri" id="righthotid"><span v-for="item in hotKeyList.right" v-tap="{ methods : loadHotKey }">{{item.name}}</span></p>
               </div>
           </transition>
        </div>
        <ui-tabs v-show="searchKeys" fullwidth text-color-active="primary" text-color="gray" ref="tabs" @tab-change="tabsChange" >
            <ui-tab title="单曲" id="tab_music" >
                <div id="tabcontent" class="tabcontent">
                    <!--单曲列表-->
                    <ul class="artist_tab tabactive" music-total="358" v-if="musicList.items.length>1">
                         <li v-for="(item, index) in musicList.items" class="singBox" @click="commPlaySong" :index="index" :mid="item.id" :songUrl="'http://antiserver.kuwo.cn/anti.s?format=aac|mp3&rid='+item.MUSICRID+'&type=convert_url&response=res'">
                            <div class="singTex">
                                <div class="singTexUp clearfix">
                                    <span class="songName fl" v-html="item.SONGNAME"></span>                                  
                                        <span class="spanSingHQ fl"></span>                                         
                                        <span v-if="item.FORMATS.match(/MV500/i)" class="spanSingLMusic fl" ></span>                                    
                                </div>
                                <div class="singName" v-html="item.ALBUM?item.ARTIST+' - '+item.ALBUM:item.ARTIST"></div>
                            </div>                          
                            <a :class="item.FORMATS.match(/mp4/i)?'singMv':''" :title="item.SONGNAME+'-'+item.ARTIST" :mid="item.MUSICRID.split('_')[1]" v-tap.stop="{ methods : showVideoPlayer }"></a>                          
            ...
            <ui-tab title="MV" id="tab_mv" @select="loadMvList">
                <div class="singList" id="searchmvid">
                    <div v-for="(item, index) in mvList.items" class="singerMv" :index="index" :title="item.SONGNAME+'-'+item.ARTIST" :mid="item.MUSICRID.split('_')[1]" v-tap="{ methods : showVideoPlayer }">
                        <p class="singerMvImg">
                            <img v-lazy="'http://img4.kwcdn.kuwo.cn:81/wmvpic/324'+item.MVPIC.slice(3)">
                        </p>
                        <p class="singerRiTex">
                            <span class="singerRiName"  v-html="item.SONGNAME"></span>
                            <span class="singerRiName2" v-html="item.ARTIST"></span>
                        </p>
                    </div>
                </div>
            </ui-tab>
    ...
    export default{
        ...
        methods:{
            ...
            showVideoPlayer(event){
                ...
                var id = event.event.currentTarget.getAttribute('mid');              
                this.$refs.mvTitle.innerHTML = event.event.currentTarget.getAttribute('title');              
                if(!this.mvViewState){
            ...
            loadHotKey(ev){
                this.$refs.searchBar.value = ev.event.target.innerText || ev.event.target.contentText;
                this.loadMusicList('true',event)
            },
            loadMusicList(isChangeKeys,event){//加载搜索出来的数据,歌曲列表
                var that = this;
                if(isChangeKeys === 'true'){                  
                    this.resetProps();
                    this.$refs.tabs.setActiveTab('tab_music')
                }              
                this.searchKeys = decodeURIComponent(this.$refs.searchBar.value)
                if(this.searchKeys == ''){                   
                    this.getHotKeys();
                    return;
                }
                ...
                this.loading = true;
                this.$http.jsonp('http://search2013.kuwo.cn/r.s?all='+this.searchKeys+'&ft=music&client=kt&cluster=0&pn='+this.musicList.currentPage+'&rn=30&rformat=json&encoding=utf8').then(function(response){
                    //this.$set(this,musicList['items'],)                  
                     if(response.body.abslist.length){
                             this.musicList.items = this.musicList.items.concat(response.body.abslist)
            ...
            loadMvList(inistTrue,event){             
                if(!this.mvList.items.length || inistTrue == 'true'){
                    this.loading = true;
                    this.$http.jsonp('http://search2013.kuwo.cn/r.s?all='+this.searchKeys+'&ft=music&newsearch=1&itemset=web_2012&client=kt&cluster=0&pn='+this.mvList.currentPage+'&rn=30&rformat=json&hasmkv=1&encoding=utf8&r=20170429').then(function(response){

                        if(response.body.abslist.length){
                            this.mvList.items = this.mvList.items.concat(response.body.abslist)
...
.newMc {
    ...
    .newBs4{
        ...
        img{
            width: 100%;
            display: inline-block;
            box-shadow: 0px 4px 13px -3px rgba(0,0,0,.9);
        }
    }
